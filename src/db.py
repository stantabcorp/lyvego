    ...
    async def insert_annonce(self, owner_id, guild_id, enable):
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("INSERT INTO annonce_notif(owner_id, guild_id, enable) VALUES(%s, %s, %s)", (owner_id, guild_id, enable, ))
                await conn.commit()
                await cur.close()
    ...
    async def update_server_role_activity(self, server_id, role_name):
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("UPDATE servers_change_role SET updated_at=%s, role_name=%s WHERE server_id=%s", (dt.datetime.now(), role_name, server_id, ))
                await conn.commit()
                await cur.close()
    ...
    async def insert_server_role_activity(self, server_id, role_name):
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("INSERT INTO servers_change_role(server_id, role_name) VALUES(%s, %s) ON DUPLICATE KEY UPDATE role_name=%s, updated_at=%s", (server_id, role_name, role_name, dt.datetime.now()))
                await conn.commit()
                await cur.close()
